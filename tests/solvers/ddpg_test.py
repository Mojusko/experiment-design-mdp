...
env = DummyTestEnvContinuous(max_episode_length = 3, min_action = -0.2, max_action = 0.2, init_state = torch.tensor([-0.5]).reshape(1, 1))
# initialize reward
reward = lambda h, s, a: torch.abs(torch.clip(s+a, -0.5, 0.5)) * -4 + 1
...
def test_policy_is_optimal():
    ...
    action0 = torch.clip(policy.ps[0](state0), -0.2, 0.2)
    action1 = torch.clip(policy.ps[1](state1), -0.2, 0.2)
    action2 = torch.clip(policy.ps[2](state2), -0.2, 0.2)
